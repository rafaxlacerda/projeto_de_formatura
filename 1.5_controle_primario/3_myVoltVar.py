...
FIG_DIR    = os.path.join(SCRIPT_DIR, "figuras_myvoltvar")
...
MAX_VOLTVAR_ITER = 1      # máximo de iterações VoltVar por hora
...
def plot_pf_activation(df: pd.DataFrame):
    """Heatmap do FP aplicado por DER (BESS e PV) e hora (azul = capacitivo, vermelho = indutivo)."""
    pivot = df.pivot_table(index="der", columns="hour", values="pf_applied", aggfunc="first")
    bess_labels = pivot.index.tolist()
    hours       = pivot.columns.tolist()

    def _to_disp(pf: float) -> float:
        if abs(pf - 1.0) < 1e-9:
            return 0.0
        elif pf > 0:  # capacitivo: pf ∈ (PF_MIN, 1) → display ∈ (0, +PF_MIN)
            return (1.0 - pf) / (1.0 - PF_MIN) * PF_MIN
        else:         # indutivo: pf ∈ (-1, -PF_MIN) → display ∈ (-PF_MIN, 0)
            return (pf + 1.0) / (1.0 - PF_MIN) * (-PF_MIN)

    display_vals = np.vectorize(_to_disp)(pivot.values)

    fig, ax = plt.subplots(figsize=(max(10, len(hours) * 0.5), max(4, len(bess_labels) * 0.6)))
    # display > 0 = capacitivo → azul; display < 0 = indutivo → vermelho; 0 = zona morta → branco
    im = ax.imshow(display_vals, cmap="RdBu", vmin=-PF_MIN, vmax=PF_MIN, aspect="auto")
    ...
    ax.set_ylabel("BESS")
    ax.set_title(f"FP aplicado pelo controle Volt-VAr (BESS + PV) — pen={PEN_PCT}% / real={ID_REALIZACAO}\n"
                 f"Azul = capacitivo (↑V, 0<pf<1)   |   Vermelho = indutivo (↓V, pf<0)   |   Branco = zona morta (pf=1)")

    for i, bess in enumerate(bess_labels):
        for j, h in enumerate(hours):
            val = pivot.loc[bess, h]
            if not np.isnan(val) and abs(val - 1.0) > 1e-6:  # não é zona morta
                ax.text(j, i, f"{val:+.2f}", ha="center", va="center",
                        fontsize=6, color="black")

    fig.colorbar(im, ax=ax, label="Azul = capacitivo (↑V)  |  Vermelho = indutivo (↓V)")
    path = os.path.join(FIG_DIR, "fig_pf_activation.png")
    fig.savefig(path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"[Plot] Heatmap FP salvo em: {path}")
...
def plot_bess_soc(df: pd.DataFrame):
    ...
    for ax, bess in zip(axes, bess_names):
        ...
        for h, s, p in zip(hours, soc, pf):
            if 0 < p < 0.999:  # capacitivo: pf positivo < 1 → injeta Q → ↑V
                ax.plot(h, s, marker="^", color="blue", markersize=8, zorder=5)
            elif p < -0.001:   # indutivo: pf negativo → absorve Q → ↓V
                ax.plot(h, s, marker="v", color="red", markersize=8, zorder=5)

        from matplotlib.lines import Line2D
        ax.legend(handles=[
            Line2D([0], [0], color="steelblue", linewidth=2, label="SOC (%)"),
            Line2D([0], [0], marker="^", color="blue", linestyle="None", markersize=8, label="FP capacitivo (↑V)"),
            Line2D([0], [0], marker="v", color="red",  linestyle="None", markersize=8, label="FP indutivo (↓V)"),
...
def main():
    ...
    # Último FP aplicado por DER — usado pelo controle secundário
    last_pf = {name: 1.0 for name in der_dict}
    ...
    for hour in range(TOTAL_HOURS):
        ...
        if voltvar_active:
            ...
            for iteration in range(MAX_VOLTVAR_ITER):
                ...
                for name, pf_desired in pf_new.items():
                    ...
                    last_pf[name] = pf_desired  # registra intenção física (independente do estado)
                ...
                print(f"  [VoltVar iter {iter_count}] violações={n_viol_new} | "
                      + " | ".join(f"{n}({der_dict[n]['type']}): V={v:.4f} pf={pf_new[n]:+.3f}"
                ...
                # Critério de parada: sem violações ou FP saturado em todos os DERs
                pf_new = {name: volt_var_pf_curve(v_new[name]) for name in der_dict}
                all_saturated = all(abs(pf) <= PF_MIN + 0.001 for pf in pf_new.values())
        ...
        for name, info in der_dict.items():
            ...
            records.append({
                ...
                "pf_applied":     round(pf_aplicado, 6),
            ...
            last_pf_records.append({
                "hour":       hour + 1,
                "der":        name,
                "der_type":   der_type,
                "pf_applied": round(pf_aplicado, 6),
            })
            status = "ativo" if pf_aplicado != 1.0 else "zona morta"
            if der_type == "BESS":
                print(f"  [BESS {name}] V_preVoltVar={v_measured[name]:.4f} | V_posVoltVar={v_final[name]:.4f} | "
                      f"pf={pf_aplicado:+.3f} ({status}) | iter={iter_count} | SOC={soc:.1f}% [{bess_state}]")
            else:
                print(f"  [PV   {name}] V_preVoltVar={v_measured[name]:.4f} | V_posVoltVar={v_final[name]:.4f} | "
                      f"pf={pf_aplicado:+.3f} ({status}) | iter={iter_count}")
    ...
    df_pf    = pd.DataFrame(last_pf_records)
    ...
    out_pf = os.path.join(CSV_DIR, f"last_pf_pen{PEN_PCT:03d}_real{ID_REALIZACAO:04d}.csv")
    df_pf.to_csv(out_pf, index=False)
    print(f"[OK] Último FP por BESS/hora salvo em: {out_pf}")
