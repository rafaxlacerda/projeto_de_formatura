# Cópia de power_flow_cenarios.py com adição da exportação dos circuitos DSS
# Para exportar, rodar no terminal: 
# python3 1.4_geracao_de_cenarios/export_monte_carlo_dss.py --exportar-dss --nivel <nivel_de_penetração>

# exemplo:
# python3 1.4_geracao_de_cenarios/export_monte_carlo_dss.py --exportar-dss --nivel 50
# exporta os circuitos do nível de penetração 50% para arquivos .dss, além de compilar os resultados em CSV e gerar o boxplot

# AVISO: se rodar com a flag --exportar-dss, sem especificar nível, exportará circuitos para TODOS os níveis de penetração

# nota R: rodei e percebi que os arquivos .dss não tão da forma mais intuitiva, 
# e o script não está salvando os .dss para todas as realizações, ainda tenho que investigar 
...
def compilar_circuito(dss_path):
    """Compila o circuito do arquivo DSS usando Compile"""
    dss.Basic.ClearAll()
    dss_path_normalized = dss_path.replace("\\", "/")
    comando = f'Compile "{dss_path_normalized}"'
    dss.Text.Command(comando)


def salvar_circuito_dss(pasta_saida, nome_arquivo):
    """
    Salva o circuito atual do OpenDSS em um arquivo .dss
    ...
    nome_arquivo : str
        Nome do arquivo (sem extensão .dss)
    ...
    # Usa o comando Save do OpenDSS para salvar o circuito
    pasta_saida_normalized = pasta_saida.replace("\\", "/")
    nome_sem_extensao = nome_arquivo.replace(".dss", "")
    comando = f'Save Circuit Dir="{pasta_saida_normalized}" File="{nome_sem_extensao}.dss"'
    dss.Text.Command(comando)
    
    caminho_salvo = os.path.join(pasta_saida, f"{nome_sem_extensao}.dss")
...
def simular_realizacao(realizacao_id, resumo, pv_df, bess_df, perfis_irr, perfis_carga, cargas_base, pasta_dss_saida=None):
    ...
    # Salvar o circuito com os elementos adicionados se pasta_dss_saida for fornecida
    caminho_dss_exportado = None
    if pasta_dss_saida:
        nome_arquivo = f"realizacao_{realizacao_id:04d}.dss"
        caminho_dss_exportado = salvar_circuito_dss(pasta_dss_saida, nome_arquivo)
    ...
    return {
        ...
        "caminho_dss_exportado": caminho_dss_exportado,
    }


def processar_nivel(pasta_nivel, dss_path, pasta_saida_nivel, max_realizacoes=None, exportar_dss=False):
    ...
    # Pasta para armazenar os arquivos DSS exportados
    pasta_dss_saida = os.path.join(pasta_saida_nivel, "circuitos_dss") if exportar_dss else None

    resultados = []
    for _, row in resumo.head(max_realizacoes).iterrows():
        id_realizacao = int(row["id_realizacao"])

        compilar_circuito(dss_path)
        ...
        resultado = simular_realizacao(
            ...
            perfis_carga,
            cargas_base,
            pasta_dss_saida=pasta_dss_saida,
        )
        ...
        if exportar_dss and resultado["caminho_dss_exportado"]:
            print(f"    → Circuito DSS salvo: {resultado['caminho_dss_exportado']}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Simula os cenários de Monte Carlo via OpenDSS, compila defeitos de tensão por faixa horária "
                    "e exporta os circuitos com elementos adicionados para arquivos .dss"
    )
    parser.add_argument(
        "--nivel",
        type=str,
    ...
    parser.add_argument(
        "--exportar-dss",
        action="store_true",
        help="Exporta os circuitos do OpenDSS com os elementos adicionados para arquivos .dss"
    )
...
def main():
    ...
    for nivel_dir in niveis:
        ...
        df_nivel = processar_nivel(pasta_nivel, dss_path, pasta_saida_nivel, max_realizacoes=args.max_realizacoes, exportar_dss=args.exportar_dss)
    ...
    if todos_resultados:
        ...
        if args.exportar_dss:
            print(f"\nCircuitos DSS exportados para: {os.path.join(pasta_saida, '*/circuitos_dss/')}")
    else:
